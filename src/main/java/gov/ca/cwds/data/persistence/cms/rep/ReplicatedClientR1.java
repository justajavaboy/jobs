...
@NamedNativeQueries({
    @NamedNativeQuery(
        ...
        query = "select c.IDENTIFIER, c.ADPTN_STCD, trim(c.ALN_REG_NO) ALN_REG_NO, c.BIRTH_DT, trim(c.BR_FAC_NM) BR_FAC_NM, "
            ...
            + "trim(c.NMPRFX_DSC) NMPRFX_DSC, c.NAME_TPC, c.OUTWRT_IND, c.P_ETHNCTYC, c.P_LANG_TPC, c.RLGN_TPC, c.S_LANG_TC, "
            + "c.SENSTV_IND, c.SNTV_HLIND, trim(c.SS_NO) SS_NO, c.SSN_CHG_CD, trim(c.SUFX_TLDSC) SUFX_TLDSC, c.UNEMPLY_CD, "
            + "c.LST_UPD_ID, c.LST_UPD_TS, trim(c.COMMNT_DSC) COMMNT_DSC, c.EST_DOB_CD, c.BP_VER_IND, c.HISP_CD, "
            + "c.CURRCA_IND, c.CURREG_IND, trim(c.COTH_DESC) COTH_DESC, c.PREVCA_IND, c.PREREG_IND, trim(c.POTH_DESC) POTH_DESC, "
            ...
            + "trim(c.CL_INDX_NO) CL_INDX_NO, c.IBMSNAP_OPERATION, c.IBMSNAP_LOGMARKER "
