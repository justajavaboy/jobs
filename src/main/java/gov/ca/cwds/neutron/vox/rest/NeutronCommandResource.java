...
public class NeutronCommandResource implements ApiMarker {
  ...
  public String masterAndCommander(@PathParam("jobName") String rocketName,
      @PathParam("command") String command, @Nonnull String body) {
    final StringBuilder buf = new StringBuilder();
    buf.append("{\"result\":\"ACK\",\"job_name\":\"").append(rocketName).append("\",\"command\":\"")
        .append(command).append("\"}");

    final VoxCommandInstruction cmd = new VoxCommandInstruction(rocketName, command);
    LOGGER.info("cmd: {}", cmd);
