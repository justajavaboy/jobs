...
DROP VIEW CWSRS1.VW_LST_CLIENT_ADDRESS
@

CREATE VIEW CWSRS1.VW_LST_CLIENT_ADDRESS (
  CLT_IDENTIFIER,
  CLT_LST_UPD_ID,
  CLT_LST_UPD_TS,
  CLT_ADJDEL_IND,
  ...
  ETH_IDENTIFIER,
  ETHNICITY_CODE,
  ETH_IBMSNAP_LOGMARKER,
  ETH_IBMSNAP_OPERATION,
  CLC_CLIENT_ID,
  CLC_GVR_ENTC,
  CLC_LST_UPD_TS,
  CLC_LST_UPD_OP,
  ...
  SAL_LST_UPD_ID,
  SAL_LST_UPD_TS,
  SAL_IBMSNAP_LOGMARKER,
  SAL_IBMSNAP_OPERATION,
  ONM_THIRD_ID, 
  ONM_FIRST_NM, 
  ONM_LAST_NM, 
  ONM_MIDDLE_NM, 
  ...
  ONM_LST_UPD_ID,
  ONM_LST_UPD_TS,
  ONM_IBMSNAP_OPERATION,
  ONM_IBMSNAP_LOGMARKER,
  CAS_IDENTIFIER, 
...
MAX(  clt.IBMSNAP_LOGMARKER, 
  ...
  NVL(sal.IBMSNAP_LOGMARKER, TIMESTAMP('2008-09-30 11:54:40')),
  NVL(onm.IBMSNAP_LOGMARKER, TIMESTAMP('2008-09-30 11:54:40')),
...
LEFT JOIN CWSRS1.SAF_ALRT    sal ON sal.FKCLIENT_T = clt.IDENTIFIER
LEFT JOIN CWSRS1.OCL_NM_T    onm ON onm.FKCLIENT_T = clt.IDENTIFIER
LEFT JOIN CWSRS1.CASE_T      cas ON cas.FKCHLD_CLT = clt.IDENTIFIER AND cas.END_DT IS NULL
WHERE CLT.IDENTIFIER IN (
	SELECT GT.IDENTIFIER FROM CWSRS1.GT_ID GT
)
@
